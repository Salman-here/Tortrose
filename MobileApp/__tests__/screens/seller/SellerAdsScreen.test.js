...
describe('SellerAdsScreen campaign rules', () => {
  ...
  it('normalizes submitted IDs, trims the note, and sends no products for stop requests', () => {
    const longNote = `  ${'a'.repeat(510)}  `;

    expect(buildAdsRequestPayload({
      requestType: 'update',
      selectedIds: ['product-1', { _id: 'product-1' }, 'product-2'],
      includeMeta: 1,
      sellerNote: longNote,
    })).toEqual({
      requestType: 'update',
